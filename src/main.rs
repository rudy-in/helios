   ...
   let mut files: Vec<_> = WalkDir::new(&dir.path)
    ...
    if file_path.is_file() {
        match fs::read_to_string(&file_path) {
            ...
            Err(_) => {
                eprintln!("{} {}", "[INFO] Skipping non-text file:".yellow(), file_path.display().to_string().red());
            }
        }
    }
}
    ...
    loop {
        terminal.draw(|f| {
            ...
            let results_widget = Paragraph::new(results.join("\n"))
        ...
        if event::poll(std::time::Duration::from_millis(250))? {
            if let Event::Key(key) = event::read()? {
                match (mode, key.code) {
                    ...
                    (Mode::Insert, KeyCode::Backspace) => { query.pop(); }
